...
)}<section className="footer-tools"><button className="key-button" onClick={() => setShowAbout(v => !v)}>About FPL Risk <span>{showAbout ? '↑' : '→'}</span></button>{showAbout && <div className="about-card"><div className="eyebrow">THE IDEA</div><h3>Understand the risk before you make the decision.</h3><p>FPL Risk helps you see how a player decision compares with managers around your global rank.</p><p>It analyses player ownership, captaincy and expected points, then compares your exposure with a relevant group of managers. From the resulting point swing, it estimates the potential effect on your rank.</p><p>The estimates are based on observed FPL data and statistical comparison. They are designed to show the likely direction and scale of risk, not guarantee your final points or rank.</p><p><strong>Free:</strong> 3 analyses per FPL gameweek. <strong>Premium:</strong> 30 analyses per FPL gameweek. <strong>Rank impact:</strong> Free for everyone.</p><div className="about-flow"><span>Your decision</span><b>→</b><span>Manager comparison</span><b>→</b><span>Point swing</span><b>→</b><span>Estimated rank impact</span></div></div>}<button className="key-button" onClick={() => setShowKey(v => !v)}>What do these numbers mean? <span>{showKey ? '↑' : '→'}</span></button>{showKey && <div className="key-grid">{KEY_ITEMS.map(([title, body]) => <div key={title}><strong>{title}</strong><p>{body}</p></div>)}</div>}</section></>}</div></main>
}
export default App
